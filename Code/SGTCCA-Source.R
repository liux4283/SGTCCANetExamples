...
SGTCCA_Net <- function(data_list, correlation_list, num_solutions = 1, 
                       prob_subsamp_prop = 0.15, prob_subsamp_num = 200,
                       common_subsamp_prop = 0.08, distinct_subsamp_prop = 0.02,
                       num_workers = 2, num_iterations = 10, network_exclude_data = NULL,
                       data_type = NULL, summary_method = 'NetSHy', pheno = NULL,
                       min_mod_size = 30, max_mod_size = 300, saving_dir = NULL, density = NULL)
{
  ...
  for (j in 1:num_solutions)
  {
    ...
    SmCCNet::networkPruning(Abar = abar[[j]],CorrMatrix = cor_mat,
                   type = types,
                   data = combined_data,
                   Pheno = pheno, ModuleIdx = j, min_mod_size = min_mod_size,
                   max_mod_size = max_mod_size, method = 'NetSHy',
                   saving_dir = saving_dir)
  }
  # save intermediate result
  save(density, cc_weight_network, abar, cor_mat, types,combined_data, pheno, file = paste0(saving_dir, '/gtcca_networkdata.Rdata'))
  cat("############################")
